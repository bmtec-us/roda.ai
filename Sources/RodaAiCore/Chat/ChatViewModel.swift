...
public final class ChatViewModel {
    ...
    private func persistMessages(
        ...
        do {
            ...
            if isNewConversation {
                ...
                // If Apple Intelligence is available, upgrade the truncated
                // fallback title to a natural-language one generated by the
                // Foundation Models framework. Runs in the background so the
                // user doesn't wait — if it fails or FM is unavailable, the
                // truncation-based title stays.
                Task { [repository, userText, conversationId] in
                    await Self.upgradeTitleIfPossible(
    ...
    /// Attempts to replace a conversation's truncation-based title with one
    /// generated by Apple Intelligence Foundation Models. No-op on devices
    /// without Apple Intelligence, and on any failure silently keeps the
    /// existing fallback title.
    private static func upgradeTitleIfPossible(
        repository: ConversationRepository,
        conversationId: UUID,
        firstUserMessage: String
    ) async {
        if #available(iOS 26, macOS 26, *) {
            guard FoundationModelHelper.isAvailable else { return }
            guard let title = await FoundationModelHelper.generateConversationTitle(
                from: firstUserMessage
            ) else {
                return
            }
            do {
                try await repository.setTitle(title, for: conversationId)
            } catch {
                print("Falha ao salvar titulo FM: \(error)")
            }
        }
    }
